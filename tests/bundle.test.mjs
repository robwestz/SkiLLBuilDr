...
before(() => {
  // --skip-build: reuse data.js generated by build.test.mjs before() hook (or data.public.js fallback)
  const r = spawnSync(process.execPath, [join(ROOT, "bundle.mjs"), "--skip-build"], { cwd: ROOT, stdio: "pipe" });
  if (r.status !== 0) {

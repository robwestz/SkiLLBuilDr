...
A local, offline browser for Claude Code skills and slash-commands — across every installed plugin, global user skills, and the current project's `.claude/`. Three modes (Browse / Compose / Recipes), a basket drawer that turns selected skills into a paste-ready prompt, and an agent-facing CLI.
...
- `tests/` — Node `node:test` smoke tests for build/query/intent/recipes (19 tests, ~500ms)
...
```bash
# After pushing to github.com/<user>/skill-browser:
#   Settings → Pages → Source = main branch, root folder
# The repo includes .nojekyll so underscore-prefixed and dot-files are served.
# landing.html becomes your public intro; link from there to skill-browser.html
```

The single-file bundle is your public app: https://<user>.github.io/skill-browser/dist/skill-browser.html
Landing page: https://<user>.github.io/skill-browser/landing.html
...
bash test.sh                  # run the test suite (19 tests)
```
